...
static struct fasync_struct *dev_async;
...
ssize_t dev_read(struct file *filp,char __user *buf,size_t count,loff_t *f_pos)
{
	ssize_t ret = 0;
	spin_lock_irq(&buffer_lock);
	if(dev_buffer.head == dev_buffer.tail && (filp->f_flags & O_NONBLOCK))
	{
		return -EAGAIN;
	}
	while((size_t)ret < count)
	{
...
{
	return fasync_helper(fd,filp,on,&dev_async);
}
